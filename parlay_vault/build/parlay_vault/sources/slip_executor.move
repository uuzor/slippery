/// Slip Executor — Slip validation and execution helpers
module parlay_vault::slip_executor {
    ...
    public fun place_slip(
        ...
        vector::push_back(&mut open_slips.slips, object::id(&receipt));
        receipt
    }
    ...
    public fun settle_won_slip(vault: &mut Vault, receipt: SlipReceipt) {
        ...
        release_payout(vault, locked_amount);
        object::delete(id);
    }
    ...
    public fun get_open_slip_count(open_slips: &OpenSlips): u64 {
        vector::length(&open_slips.slips)
    }

    public fun deepbook_predict_package(): address { DEEPBOOK_PREDICT_PKG }
}
